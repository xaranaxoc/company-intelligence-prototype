...
async function runJob(jobId, companyId, urls, settings, controller) {
  ...
  try {
    ...
    for (let i = 0; i < sourcesMeta.length; i++) {
      ...
      const result = await fetchSource(meta.url, fetcher, { maxSubpages: crawl.maxSubpages, log });
    ...
    const okCount = results.filter((r) => r.status !== 'failed' && r.status !== 'blocked').length;
    if (!okCount) throw new Error('Ни один источник не отдал данные. Проверьте ссылки — возможно, площадки закрыли доступ.');
