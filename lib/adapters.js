...
async function fetchTwogis(url, fetcher, { log, headed = true } = {}) {
  ...
  try {
    ...
    // соцсети и сайт компании из полей "url" во вложенном состоянии карточки
    ...
    const jsonUrls = [...new Set([...html.matchAll(/"url"\s*:\s*"(https?:\/\/[^"]{5,150})"/g)].map((m) => m[1].replace(/\\u002F/gi, '/')))];
    extra.socials = socialNames(jsonUrls.filter((u) => SOCIAL_RE.test(u.replace(/^https?:\/\//, '').replace(/^www\./, ''))));
    extra.website = jsonUrls.find((u) => !SOCIAL_RE.test(u.replace(/^https?:\/\//, '').replace(/^www\./, '')) && !/2gis|cdn|api\.|yandex|googl/i.test(u)) || null;
  } catch (err) {
