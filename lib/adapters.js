...
const SOCIAL_NAMES = [
  ['vk.com', 'ВКонтакте'], ['t.me', 'Telegram'], ['telegram.me', 'Telegram'], ['wa.me', 'WhatsApp'],
  ['api.whatsapp.com', 'WhatsApp'], ['ok.ru', 'Одноклассники'], ['instagram.com', 'Instagram'],
  ['max.ru', 'MAX'], ['youtube.com', 'YouTube'], ['youtu.be', 'YouTube'], ['dzen.ru', 'Дзен'],
  ['rutube.ru', 'Rutube'], ['tiktok.com', 'TikTok'], ['facebook.com', 'Facebook'], ['x.com', 'X'], ['twitter.com', 'X'],
];
function socialNames(urls) {
  const out = [];
  const byName = new Set();
  const seen = new Set();
  for (const url of urls) {
    let clean = url;
    // у wa.me 2ГИС дописывает готовый текст сообщения — убираем
    if (/^https?:\/\/(wa\.me|api\.whatsapp\.com)\//i.test(clean)) clean = clean.split('?')[0];
    const host = clean.replace(/^https?:\/\//, '').replace(/^www\./, '').split('/')[0];
    const name = SOCIAL_NAMES.find(([d]) => host === d || host.endsWith(`.${d}`))?.[1] || host;
    // одна площадка — одна ссылка: трекинг-расшифровки могут обрезаться и плодить дубли
    if (byName.has(name) || seen.has(clean)) continue;
    byName.add(name);
    seen.add(clean);
    out.push({ name, url: clean });
  }
...
async function fetchTwogis(url, fetcher, { log, headed = true } = {}) {
  ...
  const extra = { logoUrls: [], socials: [], website: null };
  try {
    ...
    // логотип организации: лежит вложенным — "logo":{"img_url":"..."}.
    // CDN отдаёт файл только с размерным суффиксом и api-version, поэтому перестраиваем URL
    const logoUrlsRaw = [...new Set([
      ...[...html.matchAll(/"logo"\s*:\s*\{\s*"img_url"\s*:\s*"([^"]+)"/g)].map((m) => m[1]),
      ...[...html.matchAll(/"logo_preview_urls"\s*:\s*\{\s*"[^"]+"\s*:\s*"([^"]+)"/g)].map((m) => m[1]),
    ])].filter((u) => /^https?:\/\//.test(u));
    extra.logoUrls = [...new Set(logoUrlsRaw.map((u) => `${u.replace(/[^/]*$/, '')}image_700x700.png?api-version=2.0`))];
    // соцсети, мессенджеры и сайт компании: из полей "url" состояния карточки
    // плюс расшифровка трекинг-ссылок link.2gis.ru (внутри — base64url настоящего URL)
    const SOCIAL_RE = /^(www\.)?(vk\.com|t\.me|telegram\.me|wa\.me|api\.whatsapp\.com|ok\.ru|instagram\.com|max\.ru|youtube\.com|youtu\.be|dzen\.ru|rutube\.ru|tiktok\.com|facebook\.com|x\.com|twitter\.com)\//i;
    const jsonUrls = [...new Set([
      ...[...html.matchAll(/"url"\s*:\s*"(https?:\/\/[^"]{5,150})"/g)].map((m) => m[1].replace(/\\u002F/gi, '/')),
      ...[...html.matchAll(/https:\/\/link\.2gis\.ru\/[A-Za-z0-9./_=-]+/g)].map((m) => {
        const seg = m[0].split('/').pop();
        try {
          const decoded = Buffer.from(seg.replace(/-/g, '+').replace(/_/g, '/'), 'base64').toString('utf8');
          return (/https?:\/\/[^\s"'\\]+/.exec(decoded) || [])[0] || null;
        } catch { return null; }
      }).filter(Boolean),
    ])];
    extra.socials = socialNames(jsonUrls.filter((u) => SOCIAL_RE.test(u.replace(/^https?:\/\//, '').replace(/^www\./, ''))));
    extra.website = jsonUrls.find((u) => !SOCIAL_RE.test(u.replace(/^https?:\/\//, '').replace(/^www\./, '')) && !/2gis|cdn|api\.|yandex|googl|link\./i.test(u)) || null;
  } catch (err) {
  ...
  const images = [];
  // логотип — первым кандидатом, чтобы медиа-пайплайн взял именно его
  for (const logoUrl of extra.logoUrls.slice(0, 2)) {
    images.push({ url: logoUrl, from: 'twogis', role: 'logo', caption: 'Логотип организации' });
  }
  images.push(...photoImages);
  ...
  const text = [
    ...
    extra.website ? `Сайт компании: ${extra.website}` : '',
    extra.socials?.length ? `Соцсети: ${extra.socials.map((s) => `${s.name} ${s.url}`).join(', ')}` : '',
  ...
  const note = status === 'ok'
    ? `Карточка получена · ${images.length} фото · ${products.length} товаров${extra.logoUrls.length ? ' · логотип' : ''}`
    : status === 'partial'
