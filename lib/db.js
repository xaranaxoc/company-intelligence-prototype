...
// ручное «нет логотипа»: убирает логотип из проекта (например, распознанный как рекламный)
export function removeLogo(companyId) {
  const files = db.prepare("SELECT file FROM media WHERE company_id = ? AND kind = 'logo'").all(companyId).map((r) => r.file);
  db.prepare("DELETE FROM media WHERE company_id = ? AND kind = 'logo'").run(companyId);
  updateCompany(companyId, { logo_file: null });
  return files;
}
