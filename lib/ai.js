...
Задача — собрать из них структурированный бриф для создания лендинга.
...
Схема ответа:
{
  ...
  "site_structure": {
    "pages": [{"name": "", "purpose": "", "sections": [{"name": "", "content": "", "priority": "high|medium|low"}]}]
  },
  "warnings": []
}
Условия качества: 3–6 страниц в site_structure; 4–8 секций на страницу «Главная»; услуги с ценами если они есть в данных; в warnings — что требует ручной проверки.`;
...
export function heuristicReport(results) {
  ...
  return {
    ...
    site_structure: defaultStructure(),
...
export function defaultStructure() {
  const sec = (name, content, priority) => ({ name, content, priority });
  return { pages: [
    { name: 'Главная', purpose: 'Быстро объяснить, кто вы и что предлагаете, и привести к целевому действию.', sections: [
      sec('Первый экран', 'Название, оффер и главная кнопка (запись/заявка)', 'high'),
      sec('Услуги', 'Сетка ключевых услуг с ценами', 'high'),
      sec('О компании', 'Короткое описание и преимущества', 'medium'),
      sec('Контакты', 'Телефон, адрес, соцсети и карта', 'high'),
    ] },
    { name: 'Услуги', purpose: 'Показать полный список услуг с ценами и условиями.', sections: [
      sec('Каталог услуг', 'Все услуги с описанием и ценой', 'high'),
      sec('Условия', 'Что входит в стоимость, материалы, время', 'medium'),
      sec('Запись', 'Кнопка или форма записи', 'high'),
    ] },
    { name: 'О компании', purpose: 'Раскрыть детали, команду и доверие.', sections: [
      sec('Подробно о компании', 'История, ценности, подход', 'medium'),
      sec('Команда', 'Мастера/сотрудники, если известны', 'low'),
      sec('Отзывы', 'Ссылки на карты и соцсети с отзывами', 'low'),
    ] },
    { name: 'Контакты', purpose: 'Дать быстрый путь к связи.', sections: [
      sec('Контактные данные', 'Телефон, мессенджеры, соцсети', 'high'),
      sec('Адрес и график', 'Адрес, режим работы, ориентиры', 'high'),
    ] },
  ] };
}
