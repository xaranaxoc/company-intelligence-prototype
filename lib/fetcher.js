...
// Общая на процесс карта «домен → время последнего запроса»:
// параллельные задания соблюдают вежливые паузы для одного домена совместно
const globalLastByHost = new Map();
...
export class Fetcher {
  ...
  async polite(host) {
    for (;;) {
      const last = globalLastByHost.get(host) || 0;
      const wait = last + this.delayMs - Date.now();
      if (wait <= 0) break;
      await sleep(wait + Math.floor(Math.random() * 300), this.signal);
    }
    globalLastByHost.set(host, Date.now());
  }
